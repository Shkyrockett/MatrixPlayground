    ...
    : IExponatable, INumericValueFactor, IGroupable, IEditable
{
    ...
    public FractionNumericMatrixFactor(double[,] items, bool editableCells = false, IExpression? exponent = null, INumeric? sequence = null, bool editable = false)
    ...
    /// <param name="sequence">The sequence.</param>
    /// <param name="editable">if set to <see langword="true" /> [editable].</param>
    public FractionNumericMatrixFactor(FractionFactor[,] items, IExpression? exponent = null, INumeric? sequence = null, bool editable = false)
    ...
    /// The items.
    /// </value>
    public FractionFactor[,] Items { get; }
    ...
    /// <summary>
    /// Gets a value indicating whether this <see cref="FractionFactor"/> is editable.
    /// </summary>
    /// <value>
        ...
        for (var row = 0; row < NumRows; row++)
        {
            for (var col = 0; col < NumCols; col++)
            {
                if (Items[row, col] is not null)
                {
                    ...
                    if (rowHeights[row] < item_size.Height)
                    {
                        rowHeights[row] = item_size.Height;
                    }

                    if (colWidths[col] < item_size.Width)
                    {
                        colWidths[col] = item_size.Width;
                    }
                }
            }
        }
